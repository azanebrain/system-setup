...
class PropertyInstruction extends instruction_1.Instruction {
    ...
    getProperties() {
        if (this.properties === undefined) {
            ...
            if (args.length === 0) {
                ...
                this.properties = [new property_1.Property(this.document, this.escapeChar, args[0], new argument_1.Argument(text, vscode_languageserver_1.Range.create(args[1].getRange().start, args[args.length - 1].getRange().end)))];
            }
            else {
                this.properties = [];
    ...
    getArguments() {
                            ...
                            whitespaceCheck: for (let j = i + 2; j < content.length; j++) {
                                switch (content.charAt(j)) {
                                    ...
                                    default:
                                        ...
                                        args.push(new argument_1.Argument(content.substring(argStart, i), vscode_languageserver_1.Range.create(this.document.positionAt(instructionNameEndOffset + start + argStart), this.document.positionAt(instructionNameEndOffset + start + i + 2))));
                                        argStart = j;
        ...
        if (argStart !== -1 && argStart !== content.length) {
            args.push(new argument_1.Argument(content.substring(argStart, content.length), vscode_languageserver_1.Range.create(this.document.positionAt(instructionNameEndOffset + start + argStart), this.document.positionAt(instructionNameEndOffset + start + content.length))));
        }
        return args;
    }
}
