...
class GitStashParser {
    static parse(data, repoPath) {
        ...
        for (let i = 0, len = entries.length; i < len; i++) {
            ...
            if (commit === undefined) {
                commit = new git_1.GitStashCommit(entry.stashName, repoPath, entry.sha, entry.fileNames, new Date(entry.date * 1000), entry.summary, undefined, entry.fileStatuses);
