...
class CommitQuickPickItem {
    constructor(commit) {
        ...
        if (commit instanceof gitService_1.GitStashCommit) {
            ...
            this.detail = `${constants_1.GlyphChars.Space} ${commit.author}, ${commit.fromNow()}${(commit.type === 'branch') ? ` ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.getDiffStatus()}` : ''}`;
        }
    }
}
