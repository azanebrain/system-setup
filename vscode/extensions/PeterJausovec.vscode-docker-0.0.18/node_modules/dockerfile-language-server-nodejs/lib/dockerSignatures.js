...
class DockerSignatures {
    ...
    getInstructionSignatures(document, instructions, position) {
        for (let instruction of instructions) {
            ...
            switch (instruction.getKeyword()) {
                ...
                case "SHELL":
                    let shell = instruction;
                    let shellSignatureHelp = {
                        signatures: [
                            {
    ...
    getFromSignatureHelp(position, from) {
        let baseImage = {
    ...
    getFromSignatures(fromSignatures, tag, digest, stagesOnly) {
        if (digest) {
    ...
    getFromActiveParameter(position, from, image, tag, digest, args) {
        const inTag = tag && docker_1.Util.isInsideRange(position, from.getImageTagRange());
        const inDigest = digest && docker_1.Util.isInsideRange(position, from.getImageDigestRange());
        const inImage = !inTag && !inDigest && docker_1.Util.isInsideRange(position, args[0].getRange());
        if (args.length === 1) {
    ...
    getJSONInstructionSignatureHelp(instruction, position, jsonSignatures, shellSignature, hasFlags, jsonFirst, singleParameter, finalRepeats) {
    ...
    getJSONSignatureActiveParameter(instruction, position, hasFlags, singleParameter, finalRepeats) {
    ...
    getSignatureActiveParameter(instruction, position, hasFlags, max, finalRepeats) {
        const flagsOffset = hasFlags ? 1 : 0;
    ...
    getPropertySignatureHelp(document, position, signatures, properties) {
        return {
    ...
    getPropertySignatures(document, position, signatures, properties) {
    ...
    getPropertySignatureActiveParameter(document, position, signatures, properties) {
        if (properties.length === 0) {
            return 0;
        }
