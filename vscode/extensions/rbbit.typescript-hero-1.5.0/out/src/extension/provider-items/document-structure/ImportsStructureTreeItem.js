...
class ImportStructureTreeItem extends BaseStructureTreeItem_1.BaseStructureTreeItem {
    ...
    getChildren() {
        ...
        if (imp instanceof typescript_parser_1.DefaultImport) {
            return [new ImportSpecifierStructureTreeItem(imp.alias, imp, this.context)];
        }
        else if (imp instanceof typescript_parser_1.ExternalModuleImport) {
            return [new ImportSpecifierStructureTreeItem(imp.alias, imp, this.context)];
        }
        else if (imp instanceof typescript_parser_1.NamedImport) {
            return imp.specifiers.map(s => new ImportSpecifierStructureTreeItem(`${s.specifier}${s.alias ? ` as ${s.alias}` : ''}`, imp, this.context));
        }
