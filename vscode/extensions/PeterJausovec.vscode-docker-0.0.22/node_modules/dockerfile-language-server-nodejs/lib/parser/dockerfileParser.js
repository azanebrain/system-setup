...
class DockerfileParser {
    static createInstruction(document, escapeChar, lineRange, instruction, instructionRange) {
        switch (instruction.toUpperCase()) {
            case "ADD":
                return new add_1.Add(document, lineRange, escapeChar, instruction, instructionRange);
            case "ARG":
                return new arg_1.Arg(document, lineRange, escapeChar, instruction, instructionRange);
            case "CMD":
                return new cmd_1.Cmd(document, lineRange, escapeChar, instruction, instructionRange);
            case "COPY":
                return new copy_1.Copy(document, lineRange, escapeChar, instruction, instructionRange);
            case "ENTRYPOINT":
                return new entrypoint_1.Entrypoint(document, lineRange, escapeChar, instruction, instructionRange);
            case "ENV":
                return new env_1.Env(document, lineRange, escapeChar, instruction, instructionRange);
            case "FROM":
                return new from_1.From(document, lineRange, escapeChar, instruction, instructionRange);
            case "HEALTHCHECK":
                return new healthcheck_1.Healthcheck(document, lineRange, escapeChar, instruction, instructionRange);
            case "LABEL":
                return new label_1.Label(document, lineRange, escapeChar, instruction, instructionRange);
            case "ONBUILD":
            ...
            case "STOPSIGNAL":
                return new stopSignal_1.StopSignal(document, lineRange, escapeChar, instruction, instructionRange);
            case "WORKDIR":
                return new workdir_1.Workdir(document, lineRange, escapeChar, instruction, instructionRange);
            case "USER":
        ...
        return new instruction_1.Instruction(document, lineRange, escapeChar, instruction, instructionRange);
    }
                ...
                case '#':
                    ...
                    for (let j = i + 1; j < buffer.length; j++) {
                        ...
                        switch (char) {
                            ...
                            case '\r':
                            case '\n':
                                return new comment_1.Comment(document, vscode_languageserver_1.Range.create(document.positionAt(commentStart), document.positionAt(j)));
                            case '=':
                                let valueStart = -1;
                                let valueEnd = -1;
                                if (directiveEnd === -1) {
                                ...
                                let lineRange = vscode_languageserver_1.Range.create(document.positionAt(commentStart), document.positionAt(lineEnd));
                                if (directiveStart === -1) {
                                ...
                                let nameRange = vscode_languageserver_1.Range.create(document.positionAt(directiveStart), document.positionAt(directiveEnd));
                                let valueRange = vscode_languageserver_1.Range.create(document.positionAt(valueStart), document.positionAt(valueEnd));
                                return new directive_1.Directive(document, lineRange, nameRange, valueRange);
                            default:
                                if (directiveStart === -1) {
                                    directiveStart = j;
                                }
    ...
    parse(document) {
        ...
        let dockerfile = new dockerfile_1.Dockerfile();
        let line = this.getDirectiveSymbol(document, buffer);
        let offset = 0;
        this.escapeChar = '\\';
        if (line instanceof directive_1.Directive) {
            let directive = line;
            dockerfile.setDirective(directive);
            if (docker_1.DIRECTIVE_ESCAPE === directive.getDirective()) {
            ...
            // skip the first line
            offset = document.offsetAt(vscode_languageserver_1.Position.create(1, 0));
        }
                ...
                case '#':
                    for (let j = i + 1; j < buffer.length; j++) {
                        ...
                        switch (char) {
                            ...
                            case '\n':
                                i = j;
                                let range = vscode_languageserver_1.Range.create(document.positionAt(i), document.positionAt(j));
                                dockerfile.addComment(new comment_1.Comment(document, range));
                                continue lineCheck;
                        }
                    }
                    // reached EOF
                    let range = vscode_languageserver_1.Range.create(document.positionAt(i), document.positionAt(buffer.length));
                    dockerfile.addComment(new comment_1.Comment(document, range));
                    break lineCheck;
                default:
                    ...
                    for (let j = i + 1; j < buffer.length; j++) {
                        ...
                        switch (char) {
                            ...
                            case '\t':
                                ...
                                for (let k = j + 1; k < buffer.length; k++) {
                                    switch (buffer.charAt(k)) {
                                        ...
                                        case '\n':
                                            ...
                                            i = k;
                                            lineRange = vscode_languageserver_1.Range.create(document.positionAt(instructionStart), document.positionAt(k));
                                            instructionRange = vscode_languageserver_1.Range.create(document.positionAt(instructionStart), document.positionAt(instructionEnd));
                                ...
                                // reached EOF
                                lineRange = vscode_languageserver_1.Range.create(document.positionAt(instructionStart), document.positionAt(buffer.length));
                                instructionRange = vscode_languageserver_1.Range.create(document.positionAt(instructionStart), document.positionAt(instructionEnd));
                                dockerfile.addInstruction(DockerfileParser.createInstruction(document, this.escapeChar, lineRange, instruction, instructionRange));
                                break lineCheck;
                            case '\r':
                            ...
                            case '\n':
                                ...
                                lineRange = vscode_languageserver_1.Range.create(document.positionAt(instructionStart), document.positionAt(instructionEnd));
