    ...
    getArguments() {
        ...
        for (let i = 0; i < fullArgs.length; i++) {
            ...
            if (docker_1.Util.isWhitespace(char)) {
                if (found !== -1) {
                    if (escapeMarker === -1) {
                        args.push(new argument_1.Argument(escapedArg, vscode_languageserver_1.Range.create(this.document.positionAt(offset + found), this.document.positionAt(offset + i))));
                    }
                    else {
                        args.push(new argument_1.Argument(escapedArg, vscode_languageserver_1.Range.create(this.document.positionAt(offset + found), this.document.positionAt(offset + escapeMarker))));
                    }
                    escapedArg = "";
                    found = -1;
                }
        ...
        if (found !== -1) {
            args.push(new argument_1.Argument(escapedArg, vscode_languageserver_1.Range.create(this.document.positionAt(offset + found), this.document.positionAt(offset + fullArgs.length))));
        }
        return args;
    }
    getVariables() {
                    ...
                    if (arg.charAt(i + 1) === '{') {
                        for (let j = i + 2; j < arg.length; j++) {
                            if (arg.charAt(j) === '}') {
                                ...
                                variables.push(new variable_1.Variable(name, vscode_languageserver_1.Range.create(this.document.positionAt(offset + i + 2), this.document.positionAt(offset + end))));
                                i = j;
                        ...
                        for (let j = i + 1; j < arg.length; j++) {
                            switch (arg.charAt(j)) {
                                ...
                                case '\'':
                                case '"':
                                    variables.push(new variable_1.Variable(arg.substring(i + 1, j), vscode_languageserver_1.Range.create(this.document.positionAt(offset + i + 1), this.document.positionAt(offset + j))));
                                    i = j - 1;
                                    continue variableLoop;
                            }
                        }
                        variables.push(new variable_1.Variable(arg.substring(i + 1, arg.length), vscode_languageserver_1.Range.create(this.document.positionAt(offset + i + 1), this.document.positionAt(offset + arg.length))));
                    }
                    break variableLoop;
            }
        }
