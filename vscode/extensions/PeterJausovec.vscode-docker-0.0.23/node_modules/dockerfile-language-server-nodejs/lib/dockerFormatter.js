...
class DockerFormatter {
    ...
    createFormattingEdit(document, start, end, indent, indentation) {
        if (indent) {
            return vscode_languageserver_1.TextEdit.replace({
                start: document.positionAt(start),
                end: document.positionAt(end)
            }, indentation);
        }
        else {
            return vscode_languageserver_1.TextEdit.del({
                start: document.positionAt(start),
                end: document.positionAt(end)
            });
        }
    ...
    formatLines(document, content, lines, indentedLines, options) {
        ...
        lineCheck: for (let line of lines) {
            let startOffset = document.offsetAt(vscode_languageserver_1.Position.create(line, 0));
                ...
                switch (content.charAt(i)) {
                    ...
                    case '\n':
                        if (i !== startOffset) {
                            ...
                            let edit = vscode_languageserver_1.TextEdit.del({
