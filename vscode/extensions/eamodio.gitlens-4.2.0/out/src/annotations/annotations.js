...
\`\`\``;
    ...
    static gutter(commit, format, dateFormatOrFormatOptions, renderOptions, compact) {
        let content = system_1.Strings.pad(gitService_1.CommitFormatter.fromTemplate(format, commit, dateFormatOrFormatOptions), 1, 1);
        if (compact) {
            content = constants_1.GlyphChars.Space.repeat(content.length);
        }
        return {
            renderOptions: {
