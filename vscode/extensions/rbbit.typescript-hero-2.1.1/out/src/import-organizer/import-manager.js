...
class ImportManager {
    constructor(document, _parsedDocument, parser, config, logger, generatorFactory) {
        ...
        this.imports = [];
        this.logger.debug(`Create import manager`, { file: document.fileName });
        this.reset();
    }
    get rootPath() {
    ...
    organizeImports() {
        this.logger.debug('Organize the imports', { file: this.document.fileName });
        this.organize = true;
        let keep = [];
    ...
    addDeclarationImport(declarationInfo) {
        this.logger.debug('Add declaration as import', { file: this.document.fileName, specifier: declarationInfo.declaration.name, library: declarationInfo.from });
        const alreadyImported = this.imports.find(o => declarationInfo.from === utility_functions_1.getAbsolutLibraryName(o.libraryName, this.document.fileName, this.rootPath) && o instanceof typescript_parser_1.NamedImport);
        if (alreadyImported) {
    ...
    commit() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            this.logger.debug('Commit the file', { file: this.document.fileName });
